...
5b. Run once, any order, all idempotent:
   ...
   - `supabase/seed_din_machines_real.sql` — DIN's real machine list from the owner: 9 areas (one per machine station — Slice/Potong/Ayak/Mixer/Press/Homogenizer/Vakum/Hand Pallet/Waterjet) + 42 machines, codes exactly as the factory writes them (A1A-KA, C11-03S, HMG1…). Same re-runnable, status-preserving shape as the SJA seed. Leaves the 4 placeholder machines from the older `seed_din_machines.sql` alone (owner's choice), so DIN lists 46.
