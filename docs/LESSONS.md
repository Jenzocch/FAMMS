...
## 七、UI 視覺驗證（2026-08 手機實測補課）

### 29. 覆寫 base 元件的寬度，必須用相同層級的 variant 前綴
- **症狀**：對話框「加寬」上線兩次，桌面上實際都還是窄的——第二次還因此把標題欄擠成一字一行的直條。
- **根本原因**：base `DialogContent` 自帶 `sm:max-w-sm`。傳入無前綴的 `max-w-3xl`，tailwind-merge **不視為衝突**（不同 variant 不互斥，兩個 class 都保留），而 CSS 裡 media query 規則後載入，sm+ 時 `sm:max-w-sm` 贏——覆寫看起來成功、實際無效。
- **修法**：覆寫一律用相同前綴（`sm:max-w-6xl` 蓋 `sm:max-w-sm`，tailwind-merge 這時才會真的替換）。
- **教訓**：**改任何 ui/ 基底元件的尺寸前，先讀它自己的 class 清單**，看要蓋的東西掛在哪個 variant 上。

### 30. 固定寬控制項排在彈性輸入框旁邊，窄容器會把輸入框壓到近零
- **症狀**：AI 預覽的標題 textarea 被壓成每行一個字的直條（約 400px 的選單/日期/按鈕把 `flex-1` 的標題擠光，auto-grow 再把它拉高）。
- **修法**：彈性欄位給硬性 `min-w-[16rem]`、控制項群組 `shrink-0`；inline 橫排只在寬度有保證的斷點（md+，配合夠寬的容器）啟用，更窄一律直向堆疊、輸入框佔滿整行。
- **教訓**：`flex-1` 沒有下限，旁邊每一個固定寬度都在吃它——排橫排前先加總固定寬度，確認最壞情況下彈性欄位還剩多少。

### 31. UI 改動，build 綠燈不算驗證
- **症狀**：同一個對話框連續兩版宣稱「修好了」，`tsc`/`eslint`/`build` 全過，實際畫面都是壞的——都是使用者一張截圖抓出來的。
- **教訓**：tsc/build 驗證的是型別和編譯，**不驗證畫面**。UI 改動的完成定義是「看到正確的畫面」（截圖或真機），不是 build 通過；環境做不到視覺驗證時，要明說「未經視覺驗證，請實測這幾點」，不宣稱修好。真人在真裝置上點一遍，抓到的問題比任何靜態分析都多——離線 UUID、送出卡住、派工名單誤導，全部都是實測抓的。
